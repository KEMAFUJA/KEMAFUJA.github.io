document.addEventListener('DOMContentLoaded', function() {
    ...
    function obtenerSugerencias(tipo, valor, listaSugerencias, inputElement) {
        ...
        const nombreTramite = "PODER";

        fetch(`../php/autocompletar2.php?tipo=${tipo}&q=${encodeURIComponent(valor)}&nombre=${encodeURIComponent(nombreTramite)}`)
    ...
    document.addEventListener('input', function(e) {
        ...
        if (target.name === 'nombre_beneficiario[]') {
            tipo = 'nombre_beneficiario';
            listaSugerencias = target.closest('.nombre_beneficiario')?.querySelector('.listaSugerenciasNombre_beneficiario');
            if (listaSugerencias) {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (target.name === 'nombre_beneficiario[]') {
            const listaSugerencias = target.closest('.nombre_beneficiario')?.querySelector('.listaSugerenciasNombre_beneficiario');
            if (listaSugerencias && listaSugerencias.children.length > 0) {
                manejarNavegacionTeclado(e, target, listaSugerencias, 'nombre_beneficiario');
            }
        }
    });
    ...
    function cerrarSugerenciasSiEsNecesario(e) {
        ...
        if (currentSuggestionsList) {
            ...
            const isnombre_beneficiarioInput = e.target.name === 'nombre_beneficiario[]';
            const isInSuggestions = e.target.closest('.listaSugerenciasNombre_beneficiario');
            
            if (!isnombre_beneficiarioInput && !isInSuggestions) {
                document.querySelectorAll('.listaSugerenciasNombre_beneficiario').forEach(lista => {
                    lista.innerHTML = '';
                });
                selectedIndex = -1;
                currentSuggestionsList = null;
    ...
    document.addEventListener('focusout', function(e) {
        if ( e.target.name === 'nombre_beneficiario[]') {
            setTimeout(() => {
