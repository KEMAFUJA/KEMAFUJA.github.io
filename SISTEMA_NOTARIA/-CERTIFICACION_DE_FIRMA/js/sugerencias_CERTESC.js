document.addEventListener('DOMContentLoaded', function() {
    ...
    function obtenerSugerencias(tipo, valor, listaSugerencias, inputElement) {
        ...
        const nombreTramite = "CERTIFICACION DE FIRMA";
    ...
    document.addEventListener('input', function(e) {
        ...
        if (target.name === 'calidad[]') {
            tipo = 'calidad';
            listaSugerencias = target.closest('.calidad')?.querySelector('.listaSugerenciascalidad');
            if (listaSugerencias) {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (target.name === 'calidad[]') {
            const listaSugerencias = target.closest('.calidad')?.querySelector('.listaSugerenciascalidad');
            if (listaSugerencias && listaSugerencias.children.length > 0) {
                manejarNavegacionTeclado(e, target, listaSugerencias, 'calidad');
            }
        }
    });
    ...
    function cerrarSugerenciasSiEsNecesario(e) {
        ...
        if (currentSuggestionsList) {
            ...
            const istipo_pagoInput = e.target === tipo_pagoInput;
            const iscalidadInput = e.target.name === 'calidad[]';
            const isInSuggestions = e.target.closest(' .listaSugerenciascalidad');
            
            if (!istipo_pagoInput && !iscalidadInput && !isInSuggestions) {
                document.querySelectorAll(' .listaSugerenciascalidad').forEach(lista => {
                    lista.innerHTML = '';
                });
                selectedIndex = -1;
                currentSuggestionsList = null;
