document.addEventListener('DOMContentLoaded', function() {
    ...
    function obtenerSugerencias(tipo, valor, listaSugerencias, inputElement) {
        ...
        fetch(`../php/autocompletar3.php?tipo=${tipo}&q=${encodeURIComponent(valor)}`)
            ...
            .then(data => {
                ...
                if (data.length > 0 && !data.error) {
                    ...
                    data.forEach((sugerencia, index) => {
                        ...
                        li.textContent = sugerencia.documento;
                        li.dataset.index = index;
                        li.dataset.value = sugerencia.documento;
    ...
    const listaSugerenciasArchivo = document.querySelector('.listaSugerenciasArchivo');
    
    if (archivoInput && listaSugerenciasArchivo) {
        let archivoTimeout;
        
        archivoInput.addEventListener('input', function() {
            clearTimeout(archivoTimeout);
            archivoTimeout = setTimeout(() => {
                obtenerSugerencias('documento', this.value, listaSugerenciasArchivo, this);
            }, 300);
        });
        
        // Evento para navegación con teclado en archivo
        archivoInput.addEventListener('keydown', function(e) {
            if (listaSugerenciasArchivo.children.length > 0) {
                manejarNavegacionTeclado(e, this, listaSugerenciasArchivo, 'documento');
            }
        });
        
        // Cerrar lista si hace click fuera
        document.addEventListener('click', function(e) {
            if (currentSuggestionsList === listaSugerenciasArchivo) {
                const clickEnInput = e.target === archivoInput;
                const clickEnLista = e.target === listaSugerenciasArchivo || 
                                     listaSugerenciasArchivo.contains(e.target);
                
                if (!clickEnInput && !clickEnLista) {
                    listaSugerenciasArchivo.innerHTML = '';
                    selectedIndex = -1;
                    currentSuggestionsList = null;
                    currentInput = null;
                }
        ...
        // Cerrar lista cuando pierde el foco
        archivoInput.addEventListener('blur', function() {
            setTimeout(() => {
                if (currentSuggestionsList === listaSugerenciasArchivo) {
                    listaSugerenciasArchivo.innerHTML = '';
                    selectedIndex = -1;
    ...
    function cerrarSugerenciasSiEsNecesario(e) {
        ...
        if (currentSuggestionsList) {
            ...
            const isArchivoInput = e.target === archivoInput;
            const isInSuggestions = e.target.closest('.listaSugerenciasArchivo');
            
            if (!isArchivoInput && !isNombreInput && !isIdentificacionInput && !isInSuggestions) {
                document.querySelectorAll('.listaSugerenciasArchivo').forEach(lista => {
                    lista.innerHTML = '';
                });
                selectedIndex = -1;
                currentSuggestionsList = null;
