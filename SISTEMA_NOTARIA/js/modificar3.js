document.addEventListener("DOMContentLoaded", function() {
    document.addEventListener("click", async function(e) {
        ...
        if (boton.classList.contains("guardar-documento")) {
            ...
            try {
                const res = await fetch("../php/editar_documento3.php", {
                    method: "POST",
                    body: datos
                });
        ...
        if (boton.classList.contains("editar-participante")) {
            ...
            fila.dataset.nombre = fila.cells[2].textContent;
            fila.dataset.identificacion = fila.cells[3].textContent;
            fila.dataset.tipo_persona = fila.cells[4].textContent;
            fila.dataset.nacionalidad = fila.cells[5].textContent;
            ...
            campos.forEach((item, i) => {
                const valor = fila.cells[i + 2].textContent;
                if (item.campo === "tipo_persona") {
                    fila.cells[i + 2].innerHTML = `
                        <select>
                    ...
                    fila.cells[i + 2].innerHTML = `<input value="${valor}" ${dataFiltroAttr} class="${item.campo}">`;
                }
            });
        ...
        if (boton.classList.contains("guardar-participante")) {
            ...
            const nombre = fila.cells[2].children[0].value;
            const identificacion = fila.cells[3].children[0].value;
            const tipo_persona = fila.cells[4].children[0].value;
            const nacionalidad = fila.cells[5].children[0].value;
            ...
            try {
                ...
                fila.cells[3].textContent = identificacion;
                fila.cells[4].textContent = tipo_persona;
                fila.cells[5].textContent = nacionalidad;
        ...
        if (boton.classList.contains("cancelar-modificar-participante")) {
            fila.cells[2].textContent = fila.dataset.nombre;
            fila.cells[3].textContent = fila.dataset.identificacion;
            fila.cells[4].textContent = fila.dataset.tipo_persona;
            fila.cells[5].textContent = fila.dataset.nacionalidad;
